...
@objc public protocol TRTCVideoCallDelegate {
    // common callback
    ...
    @objc optional func onInvited(sponsor: String, isFromGroup: Bool, userModel: VideoCallModel)
    ...
    /// 通话结束 | end callback
    @objc optional func onCallEnd(errorCode: TXLiteAVError)
    
    /// 诊所端通话结束
    @objc optional func onDoctorLeave(uid: String, hangUpType:Int, userModel:VideoCallModel)
   
    /// 解析自定义消息
       @objc optional func onCustomMessage(uid: String, customMessage:Dictionary<String,Int>, userModel:VideoCallModel)
    
    
    
    /// 网络质量
    @objc optional func onNetworkQuality(isSelf:Bool, poorNetwork:Bool);
}
