...
public class TRTCVideoCall: NSObject,
    ...
    private override init() {
        super.init()
        NotificationCenter.default.addObserver(self, selector: #selector(onNewMessage(noti:)), name: NSNotification.Name(rawValue: "VideoMessageNotification"), object: nil)
    }
    ...
    weak public var delegate:  TRTCVideoCallDelegate?
    ...
    /// user list to invite
    @objc internal func invite(userIds: [String], type: VideoCallType, groupID: String? = nil) {
    ...
    @objc public var isVideoMute: Bool = false {
        didSet {
            TRTCCloud.sharedInstance()?.muteLocalVideo(isVideoMute)
    ...
    @objc public func setVideoMute(isMute: Bool) {
        isVideoMute = isMute
    }
