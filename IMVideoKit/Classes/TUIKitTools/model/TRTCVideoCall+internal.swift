...
//                }
        ...
        case .dialing:
            ...
            if isOnCalling { // tell busy
                ...
                if let dele = delegate {
                    dele.onInvited?(sponsor: user, isFromGroup: curGroupID == nil ? false : true, userModel: model)
                }
                //被邀请方检查超时
        ...
        case .hangup:
            if curCallID == model.callid, let dele = delegate {
                ...
                dele.onDoctorLeave?(uid: user, hangUpType: model.hangUpType ,userModel: model)
            }
        case .linebusy:
            if curCallID == model.callid, let dele = delegate {
                curInvitingList = curInvitingList.filter {
        ...
        case .customMessage:
            if curCallID == model.callid, let dele = delegate {
                ...
                dele.onCustomMessage?(uid: model.callid, customMessage: model.customMessage ?? ["":0] , userModel: model)
            }
        default:
            debugPrint("📳 👻 WTF ????")
        }
    ...
    func sendModel(user: String, action: VideoCallAction,
        ...
        elem.data = data
        elem.setValue("Client_APP", forKey: "desc")
        msg.add(elem)
    ...
    func checkAutoHangUp() {
        ...
        if isInRoom && curRoomList.count == 0
            ...
            if let del = delegate {
                del.onCallEnd?(errorCode: TXLiteAVError(rawValue: 0))
            }
            hangup()
        }
    }
