...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if(self) {
        ...
        _avatarView.backgroundColor = RGB(235, 238, 245);
...
- (void)fillWithData:(TUIMessageCellData *)data
{
    ...
    [[[RACObserve(data, avatarUrl) takeUntil:self.rac_prepareForReuseSignal] ignore:nil] subscribeNext:^(NSURL *url) {
        ...
        if (![url.absoluteString isEqualToString: @""]) {
            [self.avatarView sd_setImageWithURL:url placeholderImage:self.messageData.avatarImage completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
                if (!image) {
                    [self.avatarView setImage:[UIImage imageNamed:@"IM_default_Logo"]];
                }
            }];
        }else{
            [self.avatarView setImage:[UIImage imageNamed:@"IM_default_Logo"]];
...
//    _readReceiptLabel.text = [self getReadReceiptResult];
    ...
    if(data.status == Msg_Status_Fail){
        ...
        self.retryView.image = [UIImage imageNamed:@"IM_exclamation"];
//        _readReceiptLabel.hidden = YES;
    } else {
...
//        self.avatarView.hidden = NO;
        ...
        self.retryView.mm_width(20).mm_height(20).mm__centerY(_container.mm_centerY+10).mm_left(_container.mm_maxX + 8);
//        self.readReceiptLabel.hidden = YES;
        
    } else {
//        self.avatarView.hidden = YES;
        self.avatarView.mm_w = 0;
        self.avatarView.mm_h = cellLayout.avatarSize.height;
        ...
        self.retryView.mm_width(20).mm_height(20).mm__centerY(_container.mm_centerY+10).mm_left(_container.mm_x - 8 - _indicator.mm_w);
