...
- (void)setupViews
{
    ...
    _recordImage.image = [UIImage imageNamed:@"talk1"];
...
- (void)defaultLayout
{
    ...
    _title.text = @"Swipe up to cancel";
    ...
    CGFloat imageHeight = backSize.height - titleSize.height -40;
    _recordImage.frame = CGRectMake(5, 20, backSize.width-10, imageHeight);
    _countDownLab.frame = _recordImage.frame;
    
    CGFloat titley = _recordImage.frame.origin.y + imageHeight;
    _title.frame = CGRectMake(0, titley, backSize.width, titleSize.height);
}
...
- (void)setStatus:(RecordStatus)status
{
    switch (status) {
        ...
        {
            NSLog(@"2");
            _title.text = @"Release to cancel";
            _recordImage.image = [UIImage imageNamed:@"voiceCancel"];
            _background.backgroundColor = Record_Title_Background_Color;
            break;
        }
...
{
    if ([_title.text isEqualToString:@"Release to cancel"]) {
        return;
    }
    NSString *imageName = [self getRecordImage:power];
    _recordImage.image = [UIImage imageNamed:imageName];
}
...
- (NSString *)getRecordImage:(NSInteger)power
{
    ...
    return [NSString stringWithFormat:@"talk%d", index];
}
