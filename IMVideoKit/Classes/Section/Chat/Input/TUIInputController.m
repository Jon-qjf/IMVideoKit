...
- (void)keyboardWillHide:(NSNotification *)notification
{
    if(self.clickMore){
        if (_delegate && [_delegate respondsToSelector:@selector(inputController:didChangeHeight:)]){
            [_delegate inputController:self didChangeHeight:_inputBar.frame.size.height + Bottom_SafeHeight + _moreView.frame.size.height];
            return;
        }
    }
...
//        keyboardFrame = CGRectMake(0, Screen_Height-_inputBar.frame.size.height - Bottom_SafeHeight, keyboardFrame.size.width, _inputBar.frame.size.height - Bottom_SafeHeight);
//    }
    if (_delegate && [_delegate respondsToSelector:@selector(inputController:didChangeHeight:)]){
