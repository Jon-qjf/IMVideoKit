...
- (void)setupViews
{
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(chatRevokeMsg:) name:@"CHATVC_REVOKE_IMMESSAGE" object:nil];
...
- (void)onLongPressMessage:(TUIMessageCell *)cell
{
    ...
    if ([data isKindOfClass:[TUITextMessageCellData class]]) {
        [items addObject:[[UIMenuItem alloc] initWithTitle:@"Copy" action:@selector(onCopyMsg:)]];
    }
...
//    [items addObject:[[UIMenuItem alloc] initWithTitle:@"删除" action:@selector(onDelete:)]];
    ...
    if(imMsg){
        if([imMsg isSelf] && [[NSDate date] timeIntervalSinceDate:imMsg.timestamp] < 2 * 60){
            [items addObject:[[UIMenuItem alloc] initWithTitle:@"Recall" action:@selector(onRevoke:)]];
        }
    }
...
- (void)onRetryMessage:(TUIMessageCell *)cell
{
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"Resend" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
-(void)chatRevokeMsg:(NSNotification *)noti{
    
    TUIMessageCellData * data = noti.object;
    [self revokeMsg:data];
}
