...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _textView.textColor = RGB(139, 151, 164);
...
//
//    ///对于多人，遍历以对其添加URL赋值。
//    if(data.userName.count){
//        //通过下面的函数，获取到每个昵称对应的精确位置。
//        NSArray *nameRangeList = [self findRightRangeOfAllString:data.userName inText:attributeString.string];
//        int i = 0;
...
//                                    range:nameRange];
//        }
//         NSLog(@"%@,%d,%@",data.content,data.userName.count == nameRangeList.count,data.userName);
//    }
//
//
//
...
- (BOOL)textView:(UITextView *)textView shouldInteractWithURL:(NSURL *)URL inRange:(NSRange)characterRange{
    //判定被踢或被邀请 user 时，使用 URL 判定，方便获得具体点击的目标。
        NSArray *userNames = _joinData.userName;
